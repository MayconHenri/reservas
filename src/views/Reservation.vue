<template>
  <v-container class="mx-auto" elevation="8" max-width="360">
    ...
    <v-sheet class="mx-auto" elevation="0" style="max-width: 100vw">
      <v-slide-group v-model="model" center-active show-arrows>
        <v-slide-group-item
          ...
          :key="index"
          :class="{ 'disabled-day': day.isSunday }"
          v-slot="{ isSelected, toggle }"
        >
          <v-card
            :color="isSelected ? '#343F53' : 'white'"
            class="ma-2 d-flex flex-column align-center justify-center"
            height="80"
            width="50"
            @click="!day.isSunday && toggle()"
            elevation="0"
          ...
          >
            ...
            >
              <p
                class="text-capitalize text-center font-weight-small"
    ...
    <v-tabs-window v-model="tab" class="mt-2">
      <v-tabs-window-item :value="1" v-if="!isLoading">
        <v-btn-toggle v-model="morning" class="d-flex" mandatory>
          <div class="scrollable-container">
            <v-btn
              v-for="(hourObj, index) in morningHours"
              :key="index"
              color="#343F53"
              class="mr-20 button"
              rounded="lg"
              @click="selectHour(hourObj.hour, 'morning')"
              :class="{ 'selected-btn': isSelected(hourObj.hour, 'morning') }"
              style="border: 1px solid gray"
      ...
      <v-tabs-window-item :value="2">
        ...
        >
          <div class="scrollable-container">
            <v-btn
              v-for="(hourObj, index) in afternoonHours"
              :key="index"
              color="#343F53"
              class="mr-20 button"
              rounded="lg"
              @click="selectHour(hourObj.hour, 'morning')"
              :class="{ 'selected-btn': isSelected(hourObj.hour, 'afternoon') }"
              style="border: 1px solid gray"
              :disabled="hourObj.isDisabled"
            >
              {{ hourObj.hour }}
      ...
      <v-tabs-window-item :value="3" v-if="!isLoading">
        <v-btn-toggle v-model="evening" class="d-flex" mandatory>
          <div class="scrollable-container">
            <v-btn
              v-for="(hourObj, index) in eveningHours"
              :key="index"
              color="#343F53"
              class="mr-20 button"
              rounded="lg"
              @click="selectHour(hourObj.hour, 'morning')"
              :class="{ 'selected-btn': isSelected(hourObj.hour, 'evening') }"
              style="border: 1px solid gray"
              :disabled="hourObj.isDisabled"
            >
              {{ hourObj.hour }}
    ...
    <v-dialog v-model="dialogCadastro" max-width="500px" persistent>
      <v-card>
        <v-toolbar color="#343F53" dark>
          <v-toolbar-title v-if="status === 'loading'"
            >Cadastrando...</v-toolbar-title
          >
          <v-toolbar-title v-if="status === 'success'"
            >Cadastro Concluído</v-toolbar-title
          >
          <v-toolbar-title v-if="status === 'error'"
            >Erro no Cadastro</v-toolbar-title
          >
        </v-toolbar>
  ...
  methods: {
    ...
    Valor: R$ ${this.item.price}`
      ...
      try {
        ...
        showToastSuccess("Evento criado com sucesso!")
        ...
        // Exibe toast de erro
        showToastError("Erro ao criar o evento. Tente novamente!")
    ...
    generateTimes() {
      ...
      timePeriods.forEach(
        ({ start, end, period, interval, endHour, endMinute }) => {
          const hoursArray = []
          ...
          for (let h = start; h <= endHour; h++) {
            for (let m = 0; m < 60; m += interval) {
              ...
              const isEventBlocked = this.eventos.some((event) => {
                ...
                ).subtract(this.item.time, "minute")
                ...
                return (
                  ...
                  (eventStartHour < h ||
                    (eventStartHour === h && eventStartMinute <= m)) &&
                  (eventEndHour > h ||
                    (eventEndHour === h && eventEndMinute > m))
                )
              })
              ...
              hoursArray.push({
                hour: hourFormatted,
                isDisabled: isEventBlocked || isPastTime,
              })
            }
          }
